...
class AnalysisModule(TaggableObject):
    ...
    def initialize(self, object):  # override
        # access arguments via get_init_attr(key, default)
        # self.add_tag('classifier')
        # self.add_execution_argument(...)
        # execute does not have to be used. You can just add all kinds of easy access convenience functions to the class
        return

    def execute(
        self, object, **kwargs
    ):  # override (do not call directly with instance.execute(...). Use instance(...))
        # self.parent
        return
