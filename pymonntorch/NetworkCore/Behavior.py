    ...
    def check_unused_attrs(self):
        for key in self.init_kwargs:
            if not key in self.used_attr_keys:
                print('Warning: "'+key+'" not used in set_variables of '+str(self)+' behavior! Make sure that "'+key+'" is spelled correctly and get_init_attr('+key+',...) is called in set_variables. Valid attributes are:'+str(self.used_attr_keys))

    def get_init_attr(self, key, default, object=None, do_not_diversify=False, search_other_behaviors=False, required=False):
        ...
        if key not in self.init_kwargs and object is not None and search_other_behaviors:
