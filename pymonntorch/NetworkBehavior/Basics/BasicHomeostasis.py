...
class InstantHomeostasis(Behavior):
        ...
        if th is not None:
            ...
            if gap_percent is not None:
                if has_min:
                    ...
                    check_is_torch_tensor(self.min_th, device=self.device, dtype=torch.float32)
                if has_max:
                    self.max_th += self.max_th/100*gap_percent
                    check_is_torch_tensor(self.max_th, device=self.device, dtype=torch.float32)
    ...
    def set_variables(self, object):
        ...
        self.max_th = self.get_init_attr('max_th', None, object)                           #maximum threshold for measurement param

        self.set_threshold_boundaries(
            self.get_init_attr('threshold', None, object=object),                          #threshold for measurement param (min=max=th)
            self.get_init_attr('gap_percent', None, object=object)                         #min max gap is created via a percentage from th (additional param for th)
        )

        self.distance_sensitive = self.get_init_attr('distance_sensitive', True, object)   #stronger adjustment when value is further away from optimum
        ...
        self.adjustment_param = self.get_init_attr('adjustment_param', None, object)       #name of object target attribute

        self.measurement_param = self.get_init_attr('measurement_param', None, object)     #name of parameter to be measured

        self.measurement_min = check_is_torch_tensor(
            self.get_init_attr('measurement_min', None, object),                           #minumum value which can be measured (below=0)
            device=object.device,
            dtype=torch.float32
        )
        self.measurement_max = check_is_torch_tensor(
            self.get_init_attr('measurement_max', None, object),                           #maximum value which can be measured (above=max)
            device=object.device,
            dtype=torch.float32
        )
...
class TimeIntegratedHomeostasis(InstantHomeostasis):
    def get_measurement_param(self, object):
        ...
        self.average = (self.average * self.integration_length + val) / (self.integration_length + 1)
        return self.average
